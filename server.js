...
// Due-today follow-up Telegram reminders (added 2026-08-18) — see
// server/routes.js's checkAndSendFollowUpReminders for what this actually
// does and server/telegram.js for the one-time bot setup. (This used to
// send via Resend email instead — see that function's header comment for
// why it was replaced with Telegram, then had the email fallback removed
// entirely.) Runs once shortly after boot (so a reminder due today still
// goes out even if the process was only just (re)started), then hourly
// after that. Requires this process to keep running continuously to ever
// fire — harmless but ineffective in a serverless deployment (e.g. Vercel,
// where each request spins up a fresh, short-lived instance) since the
// interval never gets the chance to tick; this is written for the
// persistent Render deployment (see render.yaml) or a long-running local
// `node server.js` session. Each call is independently try/caught and
// awaits its own ensureSeeded() so it can't run against an un-seeded store,
// and a single failure (e.g. TELEGRAM_BOT_TOKEN not configured yet) never
// crashes the timer itself.
...
async function runFollowUpReminderCheck() {
  try {
    ...
    console.error('[telegram] follow-up reminder check failed:', err.message);
  }
}
